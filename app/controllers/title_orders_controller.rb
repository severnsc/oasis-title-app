class TitleOrdersController < ApplicationController
	...
	def index
		@title_orders = TitleOrder.search(params, current_user)
	end
	...
	def title_order_params
		params.require(:title_order).permit(:id, 
		  ...
		  buyers_agent_attributes: [:id, :first_name, :last_name, :phone_number, :email, :license_number, brokerage_attributes: [:name, :license_number, address_attributes: [:street, :city, :state, :zip]]],
		  sellers_agent_attributes: [:id, :first_name, :last_name, :phone_number, :email, :license_number, brokerage_attributes: [:name, :license_number, address_attributes: [:street, :city, :state, :zip]]])
	end
end
